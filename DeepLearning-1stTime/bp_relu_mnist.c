...
void forwardPropagation(Network *network, double (*activation)(double))
{
  ...
  Layer *l_pre = &network->layer[network->n - 2];
  Layer *l_post = &network->layer[network->n - 1];
...
double updateByBackPropagation(Network *network, Neuron z[])
{
  ...
  double error = 0.;
  {
    ...
    for (int j = 0; j < output_layer->n; j++) {
      error += 0.5 * ((output_layer->z[j] - z[j]) * (output_layer->z[j] - z[j]));
    }
  }
  ...
  for (int i = 0; i < pre_c->n_post; i++) {
    ...
    for (int j = 0; j < pre_c->n_pre; j++) {
      pre_c->dw[j + pre_c->n_pre * i] += Eta
	* delta * o * (1 - o) * (pre_layer->z[j]);
      //printf("%f\n", pre_c->dw[j + pre_c->n_pre * i]);
    }
  }
  ...
  for (l = l - 1; l > 0; l--) {
	...
	o = post_layer->z[k];
	delta += post_layer->delta[k] * (o > 0)
	  * post_c->w[i + post_c->n_pre * k];
      }
      current_layer->delta[i] = delta;
      o = current_layer->z[i];
      for (int j = 0; j < pre_c->n_pre; j++) {
	pre_c->dw[j + pre_c->n_pre * i] += Eta * (o > 0)
	  * delta * (pre_layer->z[j]);
	//printf("%f\n", pre_c->dw[j + pre_c->n_pre * i]);
      }
    }
...
int main(void)
{
  ...
  for (int i = 0; i < neuron_hid; i++) {
    char fn[1024];
    sprintf(fn, "./con_png/neu_before%d.png", i);
  ...
  // generate png file with connection
  // after training
  c = &network.connection[0];
  for (int i = 0; i < neuron_hid; i++) {
    char fn[1024];
    sprintf(fn, "./con_png/neu_relu_%d.png", i);
    connection_generate_png(&c->w[i * MNIST_IMAGE_SIZE], fn);
  }
