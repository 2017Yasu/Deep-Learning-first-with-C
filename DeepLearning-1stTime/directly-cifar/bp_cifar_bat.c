...
int main(void)
{
  ...
  int num_bat = 64; // number of batch
  ...
  FILE *fp;
  char fn[1024];
  sprintf(fn, "./DAT/suc_bp_cifar_%d_bat_%d.dat", neuron_hid, epoch);
  ...
  for (int bat = 2; bat <= num_bat; bat++) {
    ...
    fprintf(fp, "%d %f\n", bat, (double)correct / CIFAR_TEST_DATA_SIZE);
