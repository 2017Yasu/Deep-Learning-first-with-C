...
static void cifar_read_training_image ( double ***training_image, int **training_label, int data_size )
{
  ...
  *training_image = (double **)malloc(data_size * sizeof(double *));
  for (int i = 0; i < data_size; i++) {
    (*training_image)[i] = (double *)malloc(CIFAR_IMAGE_SIZE * sizeof(double));
  }
...
static void cifar_read_test_image ( double ***test_image, int **test_label, int data_size )
{
  ...
  *test_image = (double **)malloc(data_size * sizeof(double *));
  for (int i = 0; i < data_size; i++) {
    (*test_image)[i] = (double *)malloc(CIFAR_IMAGE_SIZE * sizeof(double));
  }
...
{
  gdImagePtr im = gdImageCreateTrueColor(CIFAR_IMAGE_ROW_SIZE, CIFAR_IMAGE_COL_SIZE);
  int c[CIFAR_IMAGE_PIXEL];

  const int color = 256;

  for ( int i = 0; i < CIFAR_IMAGE_ROW_SIZE; i++ ) {
    for ( int j = 0; j < CIFAR_IMAGE_COL_SIZE; j++ ) {
      int r = (int)((color - 1) * data[n][j + CIFAR_IMAGE_COL_SIZE * i + 0 * CIFAR_IMAGE_PIXEL]);
      int g = (int)((color - 1) * data[n][j + CIFAR_IMAGE_COL_SIZE * i + 1 * CIFAR_IMAGE_PIXEL]);
      int b = (int)((color - 1) * data[n][j + CIFAR_IMAGE_COL_SIZE * i + 2 * CIFAR_IMAGE_PIXEL]);
      c[j + i * CIFAR_IMAGE_COL_SIZE] = gdImageColorAllocate(im, r, g, b);
    }
  }

  for ( int i = 0; i < CIFAR_IMAGE_ROW_SIZE; i++ ) {
    for ( int j = 0; j < CIFAR_IMAGE_COL_SIZE; j++ ) {
      gdImageSetPixel(im, j, i, c[j + i * CIFAR_IMAGE_COL_SIZE]);      
    }
  }
