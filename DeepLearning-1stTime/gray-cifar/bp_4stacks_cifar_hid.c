...
int main(void)
{
  ...
  const int hid_n = 4;
  int hid[hid_n];
  hid[0] = CIFAR_IMAGE_PIXEL;
  hid[1] = 64;
  hid[2] = 37;
  hid[3] = CIFAR_LABEL_SIZE;

  fprintf(stderr, "start with hid: (default %d)", hid[2]);
  while (!scanf("%d", &(hid[2]))) ;
  char fn[1024];
  sprintf(fn, "./DAT/4stacks_%d.dat", hid[2]);
  FILE *fp;
  ...
  for (int zzz = 0; zzz < 14; zzz++) {
    ...
    createLayer ( &network1, 0, hid[0] );
    createLayer ( &network1, 1, hid[1] );
    createLayer ( &network1, 2, hid[0] );
    ...
    createLayer ( &network2, 0, hid[0] );
    createLayer ( &network2, 1, hid[1] );
    createLayer ( &network2, 2, hid[2] );
    createLayer ( &network2, 3, hid[1] );
    ...
    createNetwork ( &network, 4, rng );
    createLayer ( &network, 0, hid[0] );
    createLayer ( &network, 1, hid[1] );
    createLayer ( &network, 2, hid[2] );
    createLayer ( &network, 3, hid[3] );
    ...
    copyConnection(&network2, 0, &network, 0); // copy weight of network1
    copyConnection(&network2, 1, &network, 1); // copy weight of network1
    ...
    fprintf(stderr, "%d %f\n", hid[2], (double)correct / CIFAR_TEST_DATA_SIZE);
    fprintf(fp, "%d %f\n", hid[2], (double)correct / CIFAR_TEST_DATA_SIZE);
    hid[2] += 4;
  }
  
  fprintf(stderr, "hidden layer:\n{ ");
  for (int i = 0; i < hid_n; i++) fprintf(stderr, "%d ", hid[i]);
  fprintf(stderr, "}\n");
