...
{
  // for RGB -> HEX
  gdImagePtr im = gdImageCreateTrueColor(1, 1);
  ...
  *training_label = (int *)malloc(data_size * sizeof(int));
  ...
  // to get HEX
  int *data = (int *)malloc(CIFAR_IMAGE_SIZE * sizeof(int));
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    for (int j = 0; j < CIFAR_IMAGE_PIXEL; j++) {
      int c = gdImageColorAllocate(im, data[j],
				   data[j + CIFAR_IMAGE_PIXEL],
				   data[j + 2 * CIFAR_IMAGE_PIXEL]);
      (*training_image)[i][j] = (double)c / (double)0xffffff;
    }
  }
  fclose(file);
  ...
  for (int i = 0; i < data_size; i++) {
    ...
    for (int j = 0; j < CIFAR_IMAGE_PIXEL; j++) {
      int c = gdImageColorAllocate(im, data[j],
				   data[j + CIFAR_IMAGE_PIXEL],
				   data[j + 2 * CIFAR_IMAGE_PIXEL]);
      (*test_image)[i][j] = (double)c / (double)0xffffff;
    }
  }
  fclose(file);
  gdImageDestroy(im);
...
void cifar_generate_png ( double **data, const int n, const char *filename )
{
  ...
  const int n_rgb = 0xffffff;

  for (int i = 0; i < CIFAR_IMAGE_ROW_SIZE; i++) {
    for (int j = 0; j < CIFAR_IMAGE_COL_SIZE; j++) {
      int c = (int)(data[n][j + i * CIFAR_IMAGE_COL_SIZE] * n_rgb);
      gdImageSetPixel(im, j, i, c);
    }
  }
